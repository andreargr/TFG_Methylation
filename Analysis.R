...
guardar.csv <- function(df,nombre){
  write.csv(df,nombre)
}
...
join_tn2 <- met_vector$TN2 %>% left_join(met_vector$CH2,by= c("chr", "unique.id")) %>% left_join(met_vector$`AM3-6`,by= c("chr", "unique.id")) %>% left_join(met_vector$PSA2,by= c("chr", "unique.id")) %>% left_join(met_vector$PSU2,by= c("chr", "unique.id")) %>% left_join(met_vector$ROCH2,by= c("chr", "unique.id")) %>% left_join(met_vector$TB2,by= c("chr", "unique.id"))
                  #guardado de datos
guardar.csv(num_igual_am3ch2,"coincidencias_amch.csv")
guardar.csv(num_igual_am3psa2,"coincidencias_ampsa.csv")
guardar.csv(num_igual_am3psu2,"coincidencias_ampsu.csv")
guardar.csv(num_igual_am3roch2,"coincidencias_amroch.csv")
guardar.csv(num_igual_am3tb2,"coincidencias_amchtb.csv")
guardar.csv(num_igual_am3tn2,"coincidencias_amchtn.csv")
guardar.csv(num_igual_ch2psa2,"coincidencias_chpsa.csv")
guardar.csv(num_igual_ch2psu2,"coincidencias_chpsu.csv")
guardar.csv(num_igual_ch2roch2,"coincidencias_chroch.csv")
guardar.csv(num_igual_ch2tb2,"coincidencias_chtb.csv")
guardar.csv(num_igual_ch2tn2,"coincidencias_chtn.csv")
guardar.csv(num_igual_roch2tb2,"coincidencias_rochtb.csv")
...
metilacionesunicas_tn2 <- join_tn2 %>% filter(is.na(variedad.y)) %>% filter(is.na(variedad.x.x)) %>% filter(is.na(variedad.y.y)) %>%filter(is.na(variedad.x.x.x)) %>% filter(is.na(variedad.y.y.y)) %>% filter(is.na(variedad)) %>% filter(Annotation.x !="Intergenic")
                #guardado de datos
guardar.csv(metilacionesunicas_am3,"metilacionesunicas_am.csv")
guardar.csv(metilacionesunicas_ch2,"metilacionesunicas_ch.csv")
guardar.csv(metilacionesunicas_psa2,"metilacionesunicas_psa.csv")
guardar.csv(metilacionesunicas_psu2,"metilacionesunicas_psu.csv")
guardar.csv(metilacionesunicas_roch2,"metilacionesunicas_roch.csv")
guardar.csv(metilacionesunicas_tb2,"metilacionesunicas_tb.csv")
guardar.csv(metilacionesunicas_tn2,"metilacionesunicas_tn.csv")
